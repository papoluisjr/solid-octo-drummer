...
## Drop Down Menu
def createDropD():
    print('Author: Angel Rivera-Tirado,/n prueba')

# creating basic window
window = Tk()
window.geometry("312x338") # size of the window width:- 500, height:- 375
window.resizable(0, 0) # this prevents from resizing the window
window.title("Calculator ©")
window.iconbitmap(r'calc.ico')

##---To save it as an executable(.exe), remember to savee it with (.pyw) extension--
## Drop Down Menu - continue--
menu = Menu(window)
window.config(menu=menu)

subMenu = Menu(menu)
menu.add_cascade(label="About", menu=subMenu)
subMenu.add_command(label = "Creator: A.Rivera-Tirado", command = createDropD)
#subMenu.add_command(label = "Updated by: A.Rivera-Tirado", command = createDropD)
subMenu.add_command(label = "Year: 2019", command = createDropD)

subMenu.add_separator()
subMenu.add_command(label="Close", command=menu)
...
btns_frame = Frame(window, width = 312, height = 272.5, bg = "navy")
btns_frame.pack()
